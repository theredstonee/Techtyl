...
echo "  • Server Creation for Users"
echo ""
...
# ========================================
echo ""
echo "[4/9] Downloading Pterodactyl..."
...
if [ ! -f ".env" ] || [ "$FRESH_INSTALL" = true ]; then
    ...
    echo ""
    info "Azure OpenAI Configuration:"
    echo ""
    read -p "API Key: " AZURE_KEY
    read -p "Endpoint (https://...openai.azure.com/): " AZURE_ENDPOINT
    read -p "Deployment Name (e.g. gpt-4o): " AZURE_DEPLOY
...
# ========================================
echo ""
echo "[6/9] Running database migration..."
...
# ========================================
echo ""
echo "[8/9] Installing Techtyl AI features..."
...
return [
    ...
    'user_server_creation_enabled' => env('USER_SERVER_CREATION_ENABLED', true),
    'user_server_limit' => env('USER_SERVER_LIMIT', 3),
    'user_server_memory_limit' => env('USER_SERVER_MEMORY_LIMIT', 4096),
    'user_server_cpu_limit' => env('USER_SERVER_CPU_LIMIT', 200),
    'user_server_disk_limit' => env('USER_SERVER_DISK_LIMIT', 10240),
];
PHPEOF
...
# ========================================
echo ""
echo "[9/9] Setting up user registration..."
...
class RegisterController extends Controller
{
    ...
    {
        if (!config('techtyl.registration_enabled')) {
            abort(404);
        }
...
if [ -f "resources/views/templates/wrapper.blade.php" ]; then
    if ! grep -q "techtyl-footer" resources/views/templates/wrapper.blade.php; then
        ...
        ok "Footer branding added"
    fi
fi

composer dump-autoload -q 2>/dev/null

ok "User registration configured"
...
sed -i "s|php8.2-fpm.sock|php${PHP_VER}-fpm.sock|" /etc/nginx/sites-available/pterodactyl.conf
...
echo ""
echo "✨ Features:"
echo "   ✓ User Registration: ${PANEL_URL}/auth/register"
echo "   ✓ Server Creation (3/user)"
echo "   ✓ AI Assistant"
echo ""
echo "📖 Next Steps:"
echo "   1. Open ${PANEL_URL}"
echo "   2. Login with admin account"
echo "   3. Create node (Admin → Nodes → Create New)"
echo "   4. Add allocations (e.g. port 25565)"
echo "   5. Users can register and create servers!"
echo ""
echo "🔒 Enable SSL:"
