...
# https://github.com/techtyl/techtyl-installer                                       #
#                                                                                    #
######################################################################################
...
if ! fn_exists lib_loaded; then
  ...
  source /tmp/lib.sh || source <(curl -sSL "$GITHUB_BASE_URL/lib/lib.sh")
...
main() {
  ...
  if [ -d "/var/www/pterodactyl" ]; then
    warning "The script has detected that you already have Techtyl panel on your system! You cannot run the script multiple times, it will fail!"
  ...
  MYSQL_USER="-"
  while [[ "$MYSQL_USER" == *"-"* ]]; do
    required_input MYSQL_USER "Database username (techtyl): " "" "techtyl"
  ...
  while [ -z "$timezone" ]; do
    echo -n "* Select timezone [Europe/Berlin]: "
    read -r timezone_input

    array_contains_element "$timezone_input" "${valid_timezones[@]}" && timezone="$timezone_input"
    [ -z "$timezone_input" ] && timezone="Europe/Berlin"
  done

  email_input email "Provide the email address that will be used to configure Let's Encrypt and Techtyl: " "Email cannot be empty or invalid"
...
summary() {
  print_brake 62
  output "Techtyl panel $TECHTYL_PANEL_VERSION with nginx on $OS"
...
goodbye() {
  print_brake 62
  output "Techtyl Panel installation completed"
  output ""
  ...
  output "Thank you for using Techtyl installer."
