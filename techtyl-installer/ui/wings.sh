...
export MYSQL_DBHOST_USER="techtyluser"
...
main() {
  ...
  if [ -d "/etc/pterodactyl" ]; then
    warning "The script has detected that you already have Techtyl Wings on your system! You cannot run the script multiple times, it will fail!"
  ...
  if [ "$CONFIGURE_DBHOST" == true ]; then
    ...
    while [[ "$MYSQL_DBHOST_USER" == *"-"* ]]; do
      required_input MYSQL_DBHOST_USER "Database host username (techtyluser): " "" "techtyluser"
...
function goodbye {
  echo ""
  print_brake 70
  echo "* Techtyl Wings installation completed"
  echo "*"
