...
# Techtyl versions
export TECHTYL_PANEL_VERSION=""
export TECHTYL_WINGS_VERSION=""
...
# download URLs - Using Pterodactyl base software with Techtyl branding
...
export GITHUB_BASE_URL=${GITHUB_BASE_URL:-"https://raw.githubusercontent.com/techtyl/techtyl-installer"}
...
info() {
  echo ""
  output "${COLOR_CYAN}ℹ INFO${COLOR_NC}: $1"
  echo ""
}
...
welcome() {
  ...
  echo "╚════════════════════════════════════════════════════════════════════════════╝"
  echo -e "${COLOR_NC}"
  output "Techtyl Installation Script @ $SCRIPT_RELEASE"
  output ""
  output "Based on Pterodactyl Installer"
  output "Original Copyright (C) 2018 - 2025, Vilhelm Prytz"
  output "Techtyl Adaptation (C) 2025"
  output ""
  output "Running $OS version $OS_VER."
  if [ "$1" == "panel" ]; then
    output "Latest pterodactyl/panel is $TECHTYL_PANEL_VERSION"
  elif [ "$1" == "wings" ]; then
    output "Latest pterodactyl/wings is $TECHTYL_WINGS_VERSION"
  fi
  print_brake 80
}
...
get_latest_versions() {
  ...
  TECHTYL_PANEL_VERSION=$(get_latest_release "pterodactyl/panel")
  TECHTYL_WINGS_VERSION=$(get_latest_release "pterodactyl/wings")
}
